...
function ok(name, cond, extra) {
    ...
    else { failN++; fails.push(name); console.log('  ❌ ' + name + (extra ? '  ' + extra : '')); }
}
...
async function main() {
    ...
    console.log('  导出侧真实计数:', JSON.stringify(expC));
    ...
    console.log('  导入统计:', JSON.stringify(im));
    ...
    if (hiAcc && catList.length) {
        ...
        if (other) {
            ...
            console.log('  ⚠️ 账户不足 2 个，跳过转账写入验证');
        }
    } else {
        console.log('  ⚠️ 无账户/分类，跳过交易写入验证');
    }
